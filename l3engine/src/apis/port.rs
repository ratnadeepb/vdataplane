...
impl<'a> Port<'a> {
	...
	pub fn configure(&mut self) -> Result<(), PortError> {
		...
		conf.rxmode.max_rx_pkt_len = dpdk_ffi::RTE_ETHER_MAX_LEN;
		conf.rxmode.split_hdr_size = 0;
		conf.rxmode.offloads = dpdk_ffi::DEV_RX_OFFLOAD_CHECKSUM as u64;
		...
		match unsafe { dpdk_ffi::rte_eth_dev_configure(self.id, NUM_RX_THREADS, NUM_TX_THREADS, &conf)} {
			0 => {},
			_ => return Err(PortError::new()),
		};

		// sets the port's promiscuous mode
		match unsafe { dpdk_ffi::rte_eth_promiscuous_enable(self.id) } {
			0 => {},
			_ => return Err(PortError::new()),
		};
		Ok(())
