...
impl<'a, 'b, 'c, T> Msghdr<'a, 'b, 'c, T> {
    pub fn new(
        ...
        iovv: &'b mut Vec<Iovec>,
        ctrl: &'c mut Cmsghdr<T>,
        flags: Option<c_int>,
    ) -> Msghdr<'a, 'b, 'c, T> {
...
impl<T> Cmsghdr<T> {
    pub fn new(level: c_int, scm: Scm, data: T) -> Cmsghdr<T> {
        ...
        assert_eq!(
            (size_of::<Cmsghdr<T>>() - size_of::<T>()) % size_of::<size_t>(),
            0
        );
        Cmsghdr {
...
#[allow(unused_mut)]
pub fn recvmsg<T>(
    sockfd: &mut dyn AsRawFd,
    iov_len: usize,
    mut cmsg_data: T,
) -> io::Result<(ssize_t, Vec<u8>, T)> {
...
pub fn sendmsg<T>(sockfd: &mut dyn AsRawFd, msg: &Msghdr<T>) -> io::Result<ssize_t> {
